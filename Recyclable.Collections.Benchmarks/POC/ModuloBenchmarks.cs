...
namespace Recyclable.Collections.Benchmarks.POC
{
	...
	public class ModuloBenchmarks
	{
		...
		{
			DoNothing.With(i / BlockSize, i % BlockSize);
		}

		private void CalculateWithMathDivRem(long i)
		{
			DoNothing.With(Math.DivRem(i, BlockSize, out long remainder), remainder);
		}
		...
		private void CalculateWithAddWithFor(long i)
		{
			...
			DoNothing.With(result, remainder);
		}
		...
		private void CalculateWithAddWithForOptimized(long i)
		{
			...
			DoNothing.With(result, i - total);
		}

		private void CalculateWithXDivYMulX(long i)
		{
			DoNothing.With(i / BlockSize, i - (i / BlockSize * BlockSize));
		}
		...
		private void CalculateWithXDivYMulXWithLocalVar(long i)
		{
			...
			DoNothing.With(result, i - (result * blockSize));
		}
		...
		private void CalculateWithSubtractWithWhileWithSse2(long i)
		{
			...
			DoNothing.With(state.GetElement(1), state.GetElement(0));
		}
		...
		private void CalculateWithMixedApproachWithSubtractWithWhile(long i)
		{
			...
			if (i >= _cutoff)
			{
				DoNothing.With(Math.DivRem(i, BlockSize, out remainder), remainder);
			}
			else
			{
				int result = 0;
		...
		private void CalculateWithMixedApproachWithSubtractWithWhileAsVector(long i)
		{
			...
			else
			{
				...
				DoNothing.With(state[0], state[1]);
			}
		}
		...
		private void CalculateWithMixedApproachWithSubtractWithStackalloc(long i)
		{
			...
			else
			{
				...
				DoNothing.With(state[1], state[0]);
			}
		}
		...
		private void CalculateWithMixedApproachWithForLoopOptimized(long i)
		{
			...
			if (i >= _cutoff)
			{
				DoNothing.With(Math.DivRem(i, BlockSize, out total), total);
			}
			else
			{
				total = 0;
