...
namespace Recyclable.Collections.Benchmarks.POC
{
	public partial class RecyclableListPocBenchmarks
	{
		...
		private class OldRecyclableList<T> : IDisposable, IList<T>
		{
...
#pragma warning restore CS8601
			...
			public OldRecyclableList(int minBlockSize = RecyclableDefaults.BlockSize, long? expectedItemsCount = default, ArrayPool<T[]>? memoryBlocksPool = default, ArrayPool<T>? blockArrayPool = default)
			{
			...
			public OldRecyclableList(IEnumerable<T> source, int minBlockSize = RecyclableDefaults.BlockSize, long? expectedItemsCount = default, ArrayPool<T[]>? memoryBlocksPool = default, ArrayPool<T>? blockArrayPool = default)
			{
				memoryBlocksPool ??= _defaultMemoryBlocksPool;
				blockArrayPool ??= _defaultBlockArrayPool;
				_memoryBlocksPool = memoryBlocksPool;
			...
			public void AddRange(OldRecyclableList<T> items)
			{
			...
			{
				if (source is OldRecyclableList<T> sourceRecyclableList)
				{
		...
		public void RecyclableList_EnsureCapacity_Old_ByPowOf2()
		{
			using var list = new OldRecyclableList<object>(minBlockSize: BlockSize, expectedItemsCount: TestObjectCount);
		...
		[Benchmark(Baseline = false)]
		public void RecyclableList_EnsureCapacity_Old_ByBlockSize()
		{
			using var list = new OldRecyclableList<object>();
			_ = list.EnsureCapacity(TestObjectCount);
		}
